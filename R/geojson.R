...
plot_map <- function(weatherData, iTime, estado) {
  ...
  pontos <-
    data.frame(tabela$LONGITUDE, tabela$LATITUDE, tabela[, iTime])
  ...
  new_pontos <-
    tabela[!is.na(sp::over(pontos, as(shape_estado, "SpatialPolygons"))), ]
  
  tab_raster = raster::rasterFromXYZ(new_pontos)
  r <-
    raster::raster(tab_raster, layer = grep(iTime, colnames(new_pontos)))
...
for (col in colnames(TP2M))
{
  if (col != "LATITUDE" && col != "LONGITUDE" && col > 2017082323)
  {
    ...
    for (row in 1:nrow(temperature))
    {
      ...
      date <-
        paste(paste(y, m, d, sep = "-"), paste(h, "00", "00", sep = ":"), sep =
                " ")
...
str = toJSON(
  list(type = "FeatureCollection", features = json),
  pretty = TRUE,
  auto_unbox = TRUE
)

write(str,
      paste("/var/www/html/mapbox/json/weather.geojson", sep = ""))